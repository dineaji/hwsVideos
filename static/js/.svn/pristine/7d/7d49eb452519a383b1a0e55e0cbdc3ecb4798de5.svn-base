...
(function(global,$,cookie,apiconfig){
	var fileparser,
		...
		apiBasePath = "http://beta.product.mattel.com/api/ProductInfo/GetCars",
		...
		collections = {
	        ...
	        hoverBinding : function($curEl,cbIn,cbOut,isMobile){
	        	if(isMobile) return;
	        	var self = this;
	        	$("body").on('mouseenter', $curEl, function() { 
				     self[cbIn](this)
			...
			filterAllProducts : function (carsObj, filterObj, filterMiniCollection) {
				...
				$.each(filterObj, function(targetName, el){
					...
					el.filter(function(val, index){
						...
						if(filterMiniCollection && refKey == 'MiniCollectionId') {
			...
			filterIn : function(curElem,linkingBool){
				...
				if(filtrName!="year") this[filtrName+'len'] = self[filtrName].length;
			...
			thumbnailAction : function(curElem){
				var self = this,
					...
					wishState = $curElem.find(".coll-wishlist-icon").hasClass(activeclass),
					checkElem = $curElem.find(".coll-checkmark-icon"),
					checkState = (checkElem.hasClass(activeclass)) ? activeclass : (checkElem.hasClass('success') ? "success" : ""),
				...
				filterData.wishlistState = wishState ? activeclass : "";
				filterData.checkMarkState = checkState;
			...
			checkmarkAction : function(elem){
				var $curElem = $(elem),	
					$allCurElem = $(elem).closest(".product-tile").find(".coll-checkmark-icon"),
			...
			codeValidation : function(elem,identifier){
				...
				if(inputVal==""){ errorElem.html(this.errorMessages('empty'),inputElem); }
					...
					if(identifier=="all"){
						curModel = HWLayout.getFilterData(this.carMapedDatas,'Packagingcode',inputVal)[0];
						if(typeof curModel=="undefined"){
							errorElem.html(this.errorMessages('invalid'),inputElem)
							return;	
						} 
						if(curModel.status=="verified"){
							errorElem.html(this.errorMessages('unlocked'),inputElem)
							return;
						}
						...
						setTimeout(function(){
							window.location.href = "/"+apiconfig.localeName+"/collection/detail?carId="+curModel.ToyNumber+"";
						},5000)
					} else{
						...
						this.templateBind(templeteConfig,curModel,'unlock');
						this.carMapedDatas[modelIndx].checkState = apiconfig.status= "verified";
						HWLayout.verifiedStateAPICall(curModel,curModel.ToyNumber);
					}
			...
			errorMessages: function(msg,clearTxt){
				...
				switch(msg){
					case 'empty' : 
						 msgContent = "Please fill out this field";
						 break;
					case 'invalid' : 
						 msgContent = "Oops! Try that again, please!";
						 break;
					case 'onlyNos':
						 msgContent = "Oops! Try that again, please!"
						 break;
					case 'unlocked':
						msgContent = "Oh! Already in your Cart"
						 break;
				}
				return msgContent;
			},
			...
			filterIds : function(obj,firstLoad){
				...
				for(var i=0;i<filtrRef.length;i++){
					...
					if(keyName.toLowerCase()!=="styles"){
						...
						this[nameObj] = HWLayout.filterObjects(this.filterDatas[nameObj],HWLayout.splitObjects(obj,keyName,'Id'),keyName);
					}
					this[nameObj+'len'];
			...
			detailFilter : function(elem,bool,evt){
				...
				elem = $("#"+filterId[0]+"-filter-container li[data-id="+parseInt(filterId[1])+"]");
				if(elem.length){
				...
				for(var i=0;i<filterArr.length;i++){
					...
					if(filterId!=null){
						...
						for(var j=0;j<filterIdArr.length;j++){
							filterParse = parseInt(filterIdArr[j]);
							if(typeof filterParse=="number" && !isNaN(filterParse)){
			...
			linkAnonymousUser : function(obj){
				...
				for(var i=0;i<self.carMapedDatas.length;i++){
					if(cnt){
					 if(obj['carVerified'].indexOf(self.carMapedDatas[i].ToyNumber)!=-1){
						statusname = 'verified';
						...
						cnt--;
					} else if(obj['carUnverified'].indexOf(self.carMapedDatas[i].ToyNumber)!=-1){
						statusname = 'unverified';
						self.carMapedDatas[i].status = self.carMapedDatas[i].checkState = statusname;
						$(".coll-thumb[data-id="+self.carMapedDatas[i].ToyNumber+"]").find('.coll-checkmark-icon').addClass(statusname);
						cnt--;
					} else if(obj['carWishlist'].indexOf(self.carMapedDatas[i].ToyNumber)!=-1){
						statusname = 'wishlist';
						self.carMapedDatas[i].status = self.carMapedDatas[i].checkState  = statusname;
			...
			init : function(){
				...
				// this.carDatas = HWLayout.getObjectStorage('carList') || HWLayout.ajaxResp("./ProductList.JSON");
				var self = this;
				this.filteredDatas=this.filterYear = this.filterMini = [];
				this.carAPICall();
				// this.filterDatas = HWLayout.getObjectStorage('masterAttr') || HWLayout.ajaxResp("./GetAttributes.JSON");
