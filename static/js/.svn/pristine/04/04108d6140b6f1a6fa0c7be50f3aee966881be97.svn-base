...
(function(global,$,cookie,apiconfig,Backbone){
	var fileparser,
		...
		apiBasePath = "https://beta.product.mattel.com/api/ProductInfo/GetCars",
		...
		collections = {
			...
			codeValidation : function(elem,identifier){
				...
				if(inputVal==""){ errorElem.html(this.errorMessages('empty'),inputElem); }
					...
					isGlobalCode = HWLayout.getFilterData(this.carMapedDatas,'Packagingcode',inputVal)[0];
					if((identifier=="all") || (typeof isGlobalCode!="undefined" && inputVal.toLowerCase() != curModel.Packagingcode.toLowerCase())){
						...
						HWLayout.verifiedStateAPICall(curModel,curModel.ToyNumber,curModel.MiniCollectionId);
						errorElem.html(''); 
