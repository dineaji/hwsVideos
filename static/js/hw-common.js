...
(function (global, $, cookie, apiconfig) {
    var isLoaded = false,
        ...
        layout = {
            ...
            ajaxDataFormat: function (domain, obj, cb, ObjName, isAsynOptnl, bodyObj) {
                var self = this,
            ...
            addRewardsToUser: function (bool, state, isMapped) {
                ...
                var self = this,
                    ...
                    userBadgesObj = self.rewardService.userDetails ? self.rewardService.userDetails.badges : '',
                    userSplitCategory = userBadgesObj ? this.groupUniqueObj(userBadgesObj, 'category') : '',
                    userCarsUnique = userSplitCategory['Cars collected'] ? this.uniqueVal(this.removeUndefinedArr(userSplitCategory['Cars collected']), 'badgeId') : [],
                    userCollectionUnique = userSplitCategory['Collection started'] ? this.rewardUniqueVal(this.removeUndefinedArr(userSplitCategory['Collection started']), 'collectionKey', 'badgeId') : [],
                    ...
                    isBadge = rewardRange.indexOf(self.rewardCookieLength('rewardCarCount')) != -1 ? true : false,
                    ...
                    targetObj = isMapped ? self.carRewardObj.slice(0, [badgeCnt || 1]) : (isBadge ? [self.carRewardObj[badgeCnt]] : 0),
                    isAlreadyExist = targetObj ? this.rewardCompareUserBadge(userRewardIds, targetObj) : false,
                ...
                if ((state && state.toLowerCase() != "wishlist" && state.toLowerCase() != "removed") || isMapped) {
                    var miniiDsArr = cookie.get('miniStartedIds', true, true),
                        miniObjArr = this.rewardUniqueObj(self.miniRewardObj, 'collectionKey');
                ...
                this.rewardsPointsUpdate(pointsVal, badgeTotObj, increment)
            ...
            rewardMiniStartedCookie: function (bool, toyId, optlMiniId) {
            ...
            linkcookieRewardstoUser: function () {
                if (parseInt(this.rewardCookieLength('reward_Coins_Count', true))) {
                    this.addRewardsToUser(true, null, true)
                }
            ...
            verifiedStateAPICall: function (obj, toyId, optlMiniId) {
                apiconfig.toyId = toyId;
                apiconfig.status = "verified";
                cookie.cookieName = "carVerified";
                cookie.toggleCookie(true, toyId);
                obj.status = "verified";
                this.rewardMiniStartedCookie(true, toyId, optlMiniId);
                this.rewardCoinsStore(true, 'verified');
                this.rewardCookieCnt(true, toyId, 'verified');
                this.addcarinCollectionService();
            },
            addcarinCollectionService: function () {
                if (this.ssoId) {
                    this.ajaxDataFormat(this.collService.domain(), apiconfig.apiMethodConfig(this.collService.name, 'addcar'), '', 'collService');
                }
            },
            ...
            afterSSOLogin: function () {
                var self = this;
                setTimeout(function () {
                    if (SSOAPI.Login.config.RememberMe.UserName == "EMPTY") {
                        self.afterSSOLogin();
                        return;
                    }
...
$(function () {
    ...
    $('body').on('click', '.login_scrn .login_btn', function () {
        if (isMobile) HWMAT.layout.afterSSOLogin();
    });
})
